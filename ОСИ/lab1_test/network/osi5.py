...
def network_test():
    # for i in range(1, 8):
    #     sb = run_stress_ng(f'stress-ng --netdev {i} --timeout 10')
    ...
    #                 prev_bytes = bytes
    #         sleep(1)
    #     plot_graph(timestamps, data, f'sockdiag-{i}', True)
    for i in range(1, 10, 2):
        sb = run_stress_ng(f'stress-ng --netlink-task {i} --timeout 10')
        command = 'ip -s link show eth0'
        timestamps = []
        data = []
        prev_bytes = 0
